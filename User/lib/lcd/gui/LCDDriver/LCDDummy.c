...
void LCD_L0_SetPixelIndex(int x, int y, int PixelIndex)
{
  ...
  //   {
  //     lcd->driver.set_point(lcd, xPhys, yPhys, PixelIndex);
  //   }

  // xsh: 镜像显示

  if (lcd->driver.get_dir(lcd) == 0)
  {
    int xPhys = x;
    int yPhys = y;
    lcd->driver.set_point(lcd, xPhys, yPhys, PixelIndex);
  ...
  {
    int xPhys = LCD_XSIZE - 1 - (x);
    int yPhys = LCD_YSIZE - 1 - (y);
    lcd->driver.set_point(lcd, xPhys, yPhys, PixelIndex);
  }
}
...
#define yPhys y
  ...
  {
    PixelIndex = lcd->driver.get_point(lcd, xPhys, yPhys);
  }
  return PixelIndex;
}
